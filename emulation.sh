...
source <(wget -qO- "$GAMES_DB_URL")
if [ ${#game_packs[@]} -eq 0 ]; then
    echo "ERROR: Failed to fetch or source the games list from GitHub."
...
download_pack() {
    local pack_string="$1"
    IFS=';' read -r -a pack_info <<< "$pack_string"
    local console_name="${pack_info[0]}"
    local system_id="${pack_info[1]}"
    local download_url="${pack_info[2]}"
...
run_game_installer() {
    ...
    for pack in "${game_packs[@]}"; do
        IFS=';' read -r -a pack_info <<< "$pack"
        size_kb=$(parse_size_to_kb "${pack_info[3]}")
    ...
    for i in "${!game_packs[@]}"; do
        IFS=';' read -r -a p <<< "${game_packs[$i]}"
        printf "%d) %s (%s)\n" "$((i+1))" "${p[0]}" "${p[3]}"
    done
    
    read -p "Enter number: " choice
    if [ "$choice" -gt 0 ] && [ "$choice" -le "${#game_packs[@]}" ]; then
        download_pack "${game_packs[$((choice-1))]}"
    else
...
check_for_new_packs() {
    ...
    for pack in "${game_packs[@]}"; do
        ...
        if [ ! -d "$ROMS_PATH/$system_id" ] || [ -z "$(ls -A $ROMS_PATH/$system_id)" ]; then
            ...
            read -p "Install the '${pack_info[0]}' pack? (y/n): " confirm
            if [ "$confirm" == "y" ]; then
                download_pack "$pack"
            fi
        fi
...
options=("Full Install/Update (Recommended)" "Check for New Game Packs" "Configure Kodi Integration Only" "Set Boot to Games Only" "Exit")
